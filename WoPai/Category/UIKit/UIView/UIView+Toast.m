...
- (void)makeToast:(NSString *)message {
    NSTimeInterval s = [self toastDurationForString:message];
    [self makeToast:message duration:s position:nil];
}

- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position {
    NSTimeInterval s = [self toastDurationForString:message];
    UIView *toast = [self viewForMessage:message title:nil image:nil];
    [self showToast:toast duration:s position:position];
}
...
- (NSTimeInterval)toastDurationForString:(NSString*)string {
    return MIN((float)string.length*0.06 + 1, CSToastDefaultDuration);
}
@end
